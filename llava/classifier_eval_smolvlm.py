...
# Import the classifier definition from the distillation script
from distill_llava_to_tinyvlm import SmolVLMClassifier
...
def evaluate(
    ...
    print("Loading student model backbone (SmolVLM-256M-Instruct)...")
    ...
    full_run_dir = os.path.abspath(os.path.join(output_root, run_dir))
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Eval", leave=False):
            # Note: LLaVA/SmolVLM distillation dataset use 'smol_*' keys for mapping
    ...
    try:
        ...
        plt.title("ROC Curve (SmolVLM Student - Distilled from LLaVA)")
...
def parse_args():
    ...
    p.add_argument("--run-dir", default="distill_llava_smol_20260504_004638", help="Subfolder in llava/outputs/ containing student_classifier.pt")
